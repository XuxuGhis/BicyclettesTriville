...
/*void Graphe::dessinerGrapheDijkstra(Svgfile& fichiersvg, int decalage)
{

}*/

/*///faire un dijkstra de chaque sommet
for (i=0;i<m_ordre;i++)
{
    v[i].dijkstra();
}
...
void Graphe::Dijkstra(int m_ordre, int m_taille/*vecteur de sommet et d'aretes?*/)
 {
     ...
     /* NbSommet=4;
      ...
      std::vector<std::vector<std::pair<int, int>>> G(NbSommet);                     // vecteur de graphe avec le nbdesommet
                                                                                    // chaque sommet est relié à un autre par une arete ayant un poids
      for (auto i = 0; i != NbArete; ++i)
        {
        int sommet1, sommet2, poids;
        ///sommet1= sommet 1;
        /// sommet2= sommet 2:
       /// poids = poids de larete;
      ...
      int Sommetdepart;
    ///  Sommetdepart= le sommet de départ du coup
